...
spawnPlayer()
{
	...
	else
	{
		...
		self maps\mp\gametypes\_class::giveLoadout( self.team, self.class ); //definovanie zbrane+dalsich veci....
	}
...
menuClass( response ) //SPUSTI SA PRI VYBERE CLASSU(callback)
{
	self closeMenus();
	...
	class = self maps\mp\gametypes\_class::getClassChoice( response ); //"CLASS_JAFFA;1"
	primary = self maps\mp\gametypes\_class::getWeaponChoice( response ); //0
	...
	if ( self.sessionstate == "playing" )
	{
		...
		if ( level.inGracePeriod && !self.hasDoneCombat ) // used weapons check?
		{
			...
			self maps\mp\gametypes\_class::giveLoadout( self.pers["team"], self.pers["class"] ); //STANOVENIE ZBRANE
		}
		else if ( !level.splitScreen )
		{
	...
	else
	{
		...
		if ( game["state"] == "playing" )
			self thread [[level.spawnClient]](); //STANOVENIE ZBRANE + SPAWN
	}
