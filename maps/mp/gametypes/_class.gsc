...
init()
{
	...
	//**********************************************//
	//********** Jaffa classy **********************//
	level.classMap["class1_jaffa"] = "CLASS_JAFFA;1";
	level.classMap["class2_jaffa"] = "CLASS_JAFFA;2";
...
giveLoadout( team, class )
{
	...
	*/
	...
	else if( isSubstr( class, "CLASS_JAFFA" ) )
	{	
		iprintln("start");
		// ============= jaffa classy ==============
		...
		if ( isDefined( self.pers["weapon"] ) && self.pers["weapon"] != "none" )
			...
			weapon = level.sg_class[team][class]["p_weapon"];
		
		sidearm = level.sg_class[team][class]["s_weapon"];
		...
		//health
		hp = level.sg_class[team][class]["health"];
		self.health = hp;
		self.maxhealth = hp;
		self SetNormalHealth(hp);
		
		//speed
		speed = level.sg_class[team][class]["speed"];
		self.speed = speed;
		self SetMoveSpeedScale(speed/100);
		
		//model & hands
		model = level.sg_class[team][class]["model"];
		hands = level.sg_class[team][class]["hands"];
		self SetModel(model);
		self SetViewModel(hands);
